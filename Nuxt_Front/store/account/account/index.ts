...
import {
  ...
  selectUnProSlips,
  selectCodeDetail,
  selectFinancialAccountAssociatesList,
...
export const accountStore = defineStore("accountStore", {
  state: () => ({
    ...
    controlCodeList: [] as any, // 분개 상세 코드
    detailSelectList: [] as any, // 분개 상세 셀렉트폼
  }),
  actions: {
    // 전표 전체조회
    ...
    async FETCH_SLIP(slipNo: string) {
      try {
        ...
        console.log("페치슬립", this.slipForm.slipStatus);
      } catch (error: any) {
        console.error(error);
      }
    },
    ...
    async CREATE_SLIP(slip: any) {
      try {
        ...
        console.log("resoponse", response.data.newSlipNo);
    ...
    // 20240522 분개 상세 코드 조회
    async SEARCH_CODE_DETAIL(accountCode: any) {
      console.log("일단여기까지옴", accountCode);
      try {
        const response = await selectCodeDetail(accountCode);
        console.log("리스폰된 데이터", response.data);
        console.log("이거값이 이상한가 ?", response.data.controlCodeList);
        this.controlCodeList = [...response.data.controlCodeList];
        console.log("controlCodeList 이거값이 바꼇나 ??", this.controlCodeList)
      } catch (error) {
        console.error("Error:", error);
      }
    },

    // 20240523 분개 상세 셀렉트폼 조회
    async SEARCH_DETAIL_select() {
      try {
        const response = await selectFinancialAccountAssociatesList();

        this.detailSelectList = response.data.FinancialAccountAssociatesList;
        console.log("잘실행함", this.detailSelectList);
      } catch (error) {
        console.error("Error:", error);
      }
    },

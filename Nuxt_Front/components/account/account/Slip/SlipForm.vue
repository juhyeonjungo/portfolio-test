...
<template>
    ...
    <div class="custom-search">
      <VRow>
        ...
        <VCol md="4">
          <VCardText>
            <VFormGroup>
              <VTextField v-model="searchTermDate" label="작성일자" append-inner-icon="tabler-search" type="text" />
            </VFormGroup>
          </VCardText>
        </VCol>
        <VCol md="4">
          <VCardText>
            <VFormGroup>
              <VSelect v-model="searchTermStatus" label="승인상태" placeholder="Select" :items="options" />
            </VFormGroup>
          </VCardText>
        </VCol>
      </VRow>
  ...
  <VCard>
    <div style="margin: 10px 10px">
      <MenuSlipForm :selectedSlipNo="selectedSlipNo" :selectedSlipStatus="selectedSlipStatus"
        :selectedSlipList="selectedSlipList" :getSelect="getSelect" :getSelects="getSelects" @resetSelect="resetSelect"
        @isDetailDialogClose="resetSelect" @requestSlipSelect="
                isRequestSelectSlipSnackBarVisible =
                !isRequestSelectSlipSnackBarVisible
                " @deleteFailed="deleteFailed" @deleteSlip="deleteSlip" @editSlip="editSlip" />
    </div>
    <VDataTable v-model="selectedSlip" class="text-no-wrap" :headers="column" :items="productList"
      :search="currentSearchTerm" :items-per-page="10" density="compact" show-select select-strategy="page"
      return-object />
  </VCard>
  <!-- 전표 삭제 알림 Dialog -->
  <VDialog v-model="isSlipDeleteVisible" width="400">
    ...
    <VCard title="전표삭제">
      ...
      <VCardText class="d-flex justify-end">
        <VBtn variant="outlined" @click="isSlipDeleteVisible = !isSlipDeleteVisible">
          확인
        </VBtn>
      </VCardText>
    </VCard>
  ...
  <!-- 전표 선택 요청 스낵바 -->
  <VSnackbar v-model="isRequestSelectSlipSnackBarVisible" location="top" color="primary" :timeout="1500">
    <VIcon start icon="tabler-alert-circle-filled" />
    전표를 선택해주세요.
  </VSnackbar>
  <!-- 삭제 불가 알림 스낵바 -->
  <VSnackbar v-model="isDeleteFailedSnackBarVisible" location="top" color="error" :timeout="1500">
    <VIcon start icon="tabler-alert-circle-filled" />
    승인된 전표는 삭제할 수 없습니다.
  </VSnackbar>
  <!-- 수정 불가 알림 스낵바 -->
  <VSnackbar v-model="isEditFailedSnackBarVisible" location="top" color="blue-grey" :timeout="1500">
    <VIcon start icon="tabler-alert-circle-filled" />

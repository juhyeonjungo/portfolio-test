...
const fetchData = async () => {
  ...
  productList.value = await accountStore().allSlipList;
  // filter((slip: any) => {
...
const approvalSlip = async () => {
  ...
  // table reloading
  if (
    searchTermStatus.value === "1.승인" ||
    searchTermStatus.value === "2.반려"
  )
    fetchStatusData(searchTermStatus.value.substring(2));

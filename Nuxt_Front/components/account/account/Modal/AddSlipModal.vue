...
const generateSlipNo = () => {
  // 현재 slipNo에서 숫자 부분 추출 후 숫자로 변환
  let currentNumber = parseInt(slipNo.value, 10) || 0;

  // 1 증가
  currentNumber += 1;

  // 증가된 값을 6자리 문자열로 변환 (앞에 0 채우기)
  slipNo.value = currentNumber.toString().padStart(6, "0");
};
...
//오상섭
const resetData = () => {
  accountPeriodNo.value = "6";
  generateSlipNo(); // 새로운 UUID 할당
  slipType.value = "결산";

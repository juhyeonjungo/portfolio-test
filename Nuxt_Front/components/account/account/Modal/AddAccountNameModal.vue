...
const getAccountCodeList = async () => {
  ...
  accountNameList.value = accountStore().accountCodeList;
};
...
const searchAccountCode = async () => {
  ...
  const filteredData = accountStore().accountCodeList.filter(
    (item: any) =>
      item.accountInnerCode.includes(searchValue) || item.accountName.includes(searchValue)
  );
  ...
  accountNameList.value = accountStore().accountCode;
};

const selectedRow = (selected: any, row: any) => {
  searchData.value = row.internalItem.columns.accountName;
    ...
    <VCard>
      ...
      <VRow justify="center">
        <VCol md="6" style="margin-left: 15px">
          <VTextField v-model="searchData" placeholder="계정과목을 입력하세요." @keyup.enter="searchAccountCode" />
        </VCol>
        <VCol md="5">
          <VBtnGroup color="primary" variant="flat" style="height: 42px">
            <VBtn @click="searchAccountCode" style="width: 5px">
      ...
      <VDataTable :headers="headers" :items="accountNameList" :items-per-page="8" @click:row="selectedRow" />

...
const headers = [
  ...
  { title: "계정명", key: "customerCode" },
  { title: "거래처코드", key: "customerName" },
...
const onSelected = ({ selected, row }: any) => {
  ...
  console.log( "이름들어왔나?",accountName);
};

const searchJournal = async () => {
  
  await baseStore().SEARCH_ACCOUNT_LEDGER({ accountName, startDate, endDate });
